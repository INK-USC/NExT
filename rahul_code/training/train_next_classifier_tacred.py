...
def main():
    ...
    with open("../data/result_data/dev_loss_per_epoch_Next-Clf_{}.csv".format(args.experiment_name), "w") as f:
        writer=csv.writer(f)
        writer.writerow(['strict_loss','sim_loss', 'f1_score'])
        for row in dev_epoch_losses:
            writer.writerow(row)
    
    with open("../data/result_data/test_loss_per_epoch_Next-Clf_{}.csv".format(args.experiment_name), "w") as f:
        writer=csv.writer(f)
        writer.writerow(['strict_loss','sim_loss', 'f1_score'])
